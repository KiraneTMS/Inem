...
<div class="container-fluid">
    ...
    <div class="row mb-4 align-items-end">
        <div class="col-8 breadcrumbs">
            <p> <a href="/"><i class="fa fa-home"></i> home /</a> Kelola Status</p>
            <h3>Kelola Status</h3>
        </div>
    </div>
    ...
    <div class="py-3 d-flex">
        <div class="ml-auto">
            <a href="{{ route('statuses.create') }}" class="btn btn-add">
                    ...
                    <table class="table table-bordered table-hover datatable datatable-product" cellspacing="0" width="100%">
                        ...
                        <tbody>
                            ...
                            <tr data-entry-id="{{ $status->id }}">
                                ...
                                <td>{{ $status->product->product_code }}</td>
                                <td>{{ $status->product->name }}</td>
                                <td>{{ $status->status }}</td>
                                <td>
                                    <div class="btn-group btn-group-sm">
                                        <a href="{{ route('statuses.edit', $status->id) }}" class="btn btn-info">
                                            <i class="fa fa-pencil-alt"></i>
                                        </a>
                                        <form onclick="return confirm('are you sure ? ')" class="d-inline" action="{{ route('statuses.destroy', $status->id) }}" method="POST">
                                            @csrf
                                            @method('delete')
